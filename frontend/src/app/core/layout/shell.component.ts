    ...
    <nav class="sticky top-0 z-10 bg-white border-b border-gray-200 shadow-sm">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-14">
        ...
        <div class="flex items-center gap-4">
          <span data-testid="user-email" class="text-sm text-gray-600">{{ userEmail() }}</span>
          <button
            data-testid="logout-button"
            (click)="logout()"
            class="px-3 py-1.5 text-sm font-medium text-brand-600 hover:text-brand-700 border border-brand-200 rounded-md hover:bg-brand-50 transition-colors"
          >
            Logout
          </button>
        </div>
